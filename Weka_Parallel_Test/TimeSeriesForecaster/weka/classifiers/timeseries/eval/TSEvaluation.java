...
public class TSEvaluation {
	public ArrayList<Double> anomaliesValuesPerParameter = new ArrayList<Double>(); 
	public ArrayList<Integer> anomaliesIndesPerParameter = new ArrayList<Integer>(); 
	public ArrayList<Double> anomaliesMapePerParameter = new ArrayList<Double>(); 
	public List<NumericPrediction> preds =new ArrayList<NumericPrediction>();
	public List<Double> MAPEperPred = new ArrayList<Double>();
    public DescriptiveStatistics MAPEstats = new DescriptiveStatistics();
	public double mape ;
	public double std;
    ...
    private boolean readTreshsFromFile=false;

    public void SetReadTreaholdFromFileOrGetFromCalibFunc(boolean readTreshFromFile)
    {
    	readTreshsFromFile=readTreshFromFile;
    }
    public void SetTreahold(double tresh)
    {
    	treshold=tresh;
    }
...
*/
    ...
    public void OutlierDetection(List<Double> MAPEperPred,List<NumericPrediction> preds) throws Exception {
        int cnt=0;
        int lastInd=0;
        treshold=CalcOutlierTreshold(MAPEstats);
        for (NumericPrediction p : preds){ 
    	 if (!Utils.isMissingValue(p.error()) && Math.abs(p.actual()) > 0) {
    		if (cnt<MAPEperPred.size()){
    		  if (MAPEperPred.get(cnt)>(treshold)){
    			//if (cnt!=(lastInd+1)){//in order to remove sequential anomalies 
    				anomaliesValuesPerParameter.add(p.actual());
    				anomaliesIndesPerParameter.add(cnt);
    				anomaliesMapePerParameter.add(MAPEperPred.get(cnt));
    				lastInd=cnt;
    			//}
    		  }
    		}
	...
	public ArrayList<Double> GetAnomaliesValues() throws Exception {
		return anomaliesValuesPerParameter;
	}

	public ArrayList<Integer> GetAnomaliesIndex() throws Exception {
		return anomaliesIndesPerParameter;
	}
	
	public ArrayList<Double> GetAnomaliesMape() throws Exception {
		return anomaliesMapePerParameter;
	}
	
	public Double Get_std_Measure() throws Exception {
		int truncateStd=(int)(std*100);
		std=((double) truncateStd)/100;
		return std;
	}
	public Double Get_median_Measure() throws Exception {
		int truncateMedian=(int)(median*100);
		median=((double) truncateMedian)/100;
		return median;
	}
	public Double Get_treshold_Measure() throws Exception {
		int truncateTreshold=(int)(treshold*100);
		treshold=((double) truncateTreshold)/100;
		return treshold;
	}
	public Double Get_MAPE_Measure() throws Exception {
		int truncateMape=(int)(mape*100);
		mape=((double) truncateMape)/100;
		return mape;
	}
	public Double Get_upperPercentile_Measure() throws Exception {
		int truncateUpperPercentile=(int)(upperPercentile*100);
		upperPercentile=((double) truncateUpperPercentile)/100;
		return upperPercentile;
	}
	public void CalculateAnomalies() throws Exception
	{
        preds = null; 
        MAPEperPred  =null;
        MAPEstats  =null;
	    ...
	    if (m_evaluateTrainingData) {
	      ...
	      for (String key : keys) {
	        ...
	        for (TSEvalModule e : evalsForKey) {
	          ...
	          preds = e.preds; 
	          MAPEperPred  = e.MAPEperPred;
	          MAPEstats  = e.MAPEstats;
	         // treshold=CalcOutlierTreshold(MAPEstats);
	    ...
	    if (m_evaluateTestData) {
	      ...
	      for (String key : keys) {
	        ...
	        for (TSEvalModule e : evalsForKey) {
	          ...
	          MAPEstats  = e.MAPEstats;
		      // treshold=CalcOutlierTreshold(MAPEstats);
	          OutlierDetection(MAPEperPred,preds);
	          for (double m : metricsForTargets) {
	            maxWidth = maxWidth(maxWidth, m);
	          }
	        }
    ...
    if (forecaster instanceof TSLagUser) {
      ...
      if (m_evaluateTestData) {
        ...
        if (testMod.numInstances() != m_testData.numInstances()) {
         // m_testData = testMod; //deddy commented this line because replaceMissing() generates more instances and not only interpolate and replace missing instances
        //TODO :fix this bug or do data preprocess with this functionality
        }
      }
