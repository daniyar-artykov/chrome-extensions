...
var rumola = {
	...
	document_load: function(pd) {
		...
		if ((""+document.domain).match(/(rumola\.com|skipinput\.com)$/)) {
	...
	response_from_first_gate: function(tags, b_gate_url) {
		...
		{
			if (!(rumola.document.my_forms[tags[i-3]] instanceof rumola_form_wait))
				continue;

			if (tags[i-3] == 0) {
				if ((rumola.document.my_forms_array_type == 0)&&(rumola.document.zero_form_tag != tags[i-4]))
					return;

				rumola.document.leave_only_zero_form();
			}

			rumola_prefs.last_found_captcha_id = tags[i-3];
			console.log('last_found_captcha_id ' + last_found_captcha_id);
			var tmp = new rumola_form_ready(rumola.document.my_forms[tags[i-3]].my_form,
				rumola.document.my_forms[tags[i-3]].important_elements[tags[i-2]],
				rumola.document.my_forms[tags[i-3]].important_elements[tags[i-1]],
				tags[i], b_gate_url);
