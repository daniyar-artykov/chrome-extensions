...
//			alert(login + " | " + password + " | " + retry + " | " + useCredentials);
		...
		$scope.restoreOnline = function() {
			...
			return $http({
				...
				url: 'http://proxee.net/files/proxee.bak',
				cache: false,
				timeout: 10000,
				responseType: "text"
...
//				alert('multiRoutes: ' + multiRoutes + '; nonProxiedUdp: ' + nonProxiedUdp);
		...
		$rootScope.restoreMultiRoutesOption = function() {
			...
			chrome.privacy.network.webRTCMultipleRoutesEnabled.get({},
				...
				$rootScope.restoreRadios(multiRoutes, true);
				$rootScope.restoreNonProxiedUdpOption(multiRoutes);
			});
		}
		...
		$scope.$on('$viewContentLoaded', $rootScope.restoreMultiRoutesOption);
		$scope.profileIcons = profileIcons;
		$scope.dispNameFilter = dispNameFilter;
